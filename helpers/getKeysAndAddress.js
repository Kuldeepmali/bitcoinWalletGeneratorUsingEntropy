...
const generateMnemonicUsingEntropy = require('./generateMnemonicsUsingEntropy')

const getKeysAndAddress = async (entropy, derivationPath, callback) => {
  try {
    if (!bippath.validateString(derivationPath)) {
      throw new TypeError('Invalid derivationPath')
    }
    const mnemonic = await generateMnemonicUsingEntropy(entropy)
